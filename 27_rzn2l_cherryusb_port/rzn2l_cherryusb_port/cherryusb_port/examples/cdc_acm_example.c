...
{
    LOG_INFO("\r\n=== USB CDC ACM ===\r\n");
    LOG_INFO("  Module:  USB_IP0\r\n");
    LOG_INFO("  Mode:    Peripheral\r\n");
#ifdef CONFIG_USB_HS
    LOG_INFO("  Speed:   High-Speed (480 Mbps)\r\n");
#else
    LOG_INFO("  Speed:   Full-Speed (12 Mbps)\r\n");
#endif
    LOG_INFO("  VID:     0x%04X\r\n", USB_DEV_VID);
    LOG_INFO("  PID:     0x%04X\r\n", USB_DEV_PID);
    LOG_INFO("  EP IN:   0x%02X max=%u\r\n", CDC_ACM_EP_IN_ADDR, CDC_ACM_EP_IN_MAX_PACKET);
    LOG_INFO("  EP OUT:  0x%02X max=%u\r\n", CDC_ACM_EP_OUT_ADDR, CDC_ACM_EP_OUT_MAX_PACKET);
    LOG_INFO("  Buffer:  RX=%u TX=%u\r\n", CDC_ACM_RX_BUFFER_SIZE, CDC_ACM_TX_BUFFER_SIZE);
    print_mode_info();
}
