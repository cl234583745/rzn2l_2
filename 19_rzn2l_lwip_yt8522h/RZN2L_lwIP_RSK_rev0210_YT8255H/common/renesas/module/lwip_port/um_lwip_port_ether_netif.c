										 ...
										 ether_netif_instance_t const * const p_ether_netif_instance)
{
    ...
    usr_err = p_ctrl->p_ether_netif_instance->p_api->callbackAdd( (ether_netif_ctrl_t * const) p_ctrl->p_ether_netif_instance->p_ctrl, &p_ctrl->callback_node );
...
void _ether_netif_callback( ether_netif_callback_args_t * p_args )
{
    ...
    switch( p_args->event )
    {
	case ETHER_NETIF_CALLBACK_EVENT_LINK_UP:
	...
	case ETHER_NETIF_CALLBACK_EVENT_LINK_DOWN:
