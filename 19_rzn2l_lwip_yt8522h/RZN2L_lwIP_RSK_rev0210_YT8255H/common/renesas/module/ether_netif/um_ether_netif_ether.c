...
static usr_err_t _fetch_rx_management_tag ( ether_netif_ether_ctrl_t * const p_ctrl, struct pbuf *p_frame, ether_mgtag_t const * p_mgtag );
static usr_err_t _insert_management_tag ( ether_netif_frame_t * const, ether_mgtag_t const * p_mgtag );
static usr_err_t _remove_management_tag ( struct pbuf *p_frame, ether_mgtag_t * p_mgtag );
...
static ether_netif_ether_ctrl_t *pg_ether_if;
/***********************************************************************************************************************
 * Functions
 **********************************************************************************************************************/
/*******************************************************************************************************************//**
 ...
 **********************************************************************************************************************/
usr_err_t um_ether_netif_ether_read( ether_netif_ether_ctrl_t * const p_ctrl, struct pbuf *p_frame )
{
    /** Error codes */
    fsp_err_t fsp_err;

    ePort_index = (uint8_t)ETHER_NETIF_PORT_RECV_PORT_ANY;
    /** Initialize the port number */
    p_frame->if_idx = ePort_index;
    ...
    {
        /** Read Ethernet frame. */
        fsp_err = p_ctrl->p_ether_instance->p_api->read(p_ctrl->p_ether_instance->p_ctrl, p_frame->payload, (uint32_t *)&p_frame->len);
 ...
 * @retval USR_ERR_IN_USE		Resources are busy.
 **********************************************************************************************************************/
usr_err_t um_ether_netif_ether_write(struct netif * const p_ctrl, struct pbuf *p_packet )
{
	/** Error codes */
	...
	xSemaphoreTake( pg_ether_if->p_mutex_handle , portMAX_DELAY);

	/** Get the head address of frame buffer */
    p_concat_frame = ether_netif_frame.p_buffer;
    ...
    ether_netif_frame.port = (uint32_t)ePort_index;
    /** The received Ethernet frame does not have the management tag. */
    if( ETHSW_SPECIFIC_TAG_ENABLE == gmac_extend_cfg->specific_tag )
    {
        (void) _build_tx_management_tag((ether_netif_ether_ctrl_t * )p_ctrl, &ether_netif_frame, &mgtag);
        (void) _insert_management_tag(&ether_netif_frame, &mgtag);
    }
    ...
    //USR_LOCK_SECTION_START( p_ctrl->p_mutex_handle );
    {
        ...
        fsp_err = pg_ether_if->p_ether_instance->p_api->write(pg_ether_if->p_ether_instance->p_ctrl, ether_netif_frame.p_buffer, ether_netif_frame.length );
        //USR_LOCK_ERROR_RETURN( p_ctrl->p_mutex_handle , FSP_SUCCESS == fsp_err, USR_ERR_ABORTED);
    }
    //USR_LOCK_SECTION_END( p_ctrl->p_mutex_handle );
   xSemaphoreGive(pg_ether_if->p_mutex_handle);
...
static usr_err_t _build_tx_management_tag ( ether_netif_ether_ctrl_t * const p_ctrl, ether_netif_frame_t * const p_frame, ether_mgtag_t * p_mgtag )
{
    ...
    p_mgtag->control_tag[0]  = (uint8_t)( ( gmac_extend_cfg->specific_tag_id >> 8 ) & 0x00FF ) ;
    p_mgtag->control_tag[1]  = (uint8_t)( ( gmac_extend_cfg->specific_tag_id      ) & 0x00FF ) ;
...
static usr_err_t _fetch_rx_management_tag ( ether_netif_ether_ctrl_t * const p_ctrl, struct pbuf *p_frame, ether_mgtag_t const * p_mgtag )
{
    ...
    /** Load control data */
    uint32_t control_data = (uint32_t) ( p_mgtag->control_data[1] << 8U ) + ( p_mgtag->control_data[0] );

    /** Get port where the received frame was received from. */
    p_frame->if_idx = ( (0x01) << (control_data & GMAC_RX_MGTAG_DATA1_PORT_MASK) );
...
{
    /** Resolve the Ethernet frame as it has a management tag. */
    ether_frame_mgtag_t * p_mgtag_frame = (ether_frame_mgtag_t *) p_frame->payload;

    /** Resolve the Ethernet frame as it does not have a management tag. */
    ether_frame_t * p_ether_frame = (ether_frame_t* ) p_frame->payload;
    ...
    memmove( &p_ether_frame->type, &p_mgtag_frame->type,
             p_frame->len - ( sizeof(p_mgtag_frame->dst_addr) + sizeof(p_mgtag_frame->src_addr) + sizeof(p_mgtag_frame->mgtag) ) );

    /** Subtract the length of management tag. */
    p_frame->len =  p_frame->len - sizeof(ether_mgtag_t);
